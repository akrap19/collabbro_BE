...
const paths = {
  ...
  '/auth/password/reset': {
    post: {
      tags: ['Auth'],
      description:
        'Reset password. User will not be able to login with third party login anymore.',
      requestBody: {
        content: {
          'application/json': {
            schema: {
...
const definitions = {
  ...
  auth_login_body: {
    type: 'object',
    description:
      'Password and email are required if authType is Password. If authType is Google, Facebook or LinkedIn, token should be provided.',
    properties: {
      email: {
        type: 'string',
      ...
      token: {
        type: 'string',
        example: '3g4txgxzg4xhhx'
      },
      authType: {
        type: 'string',
        enum: ['Google', 'Facebook', 'LinkedIn', 'Password']
      }
    },
